...
| 缺点                       |                                                              | 客户端和服务端需要各自维护一个子线程监听数据传输<br>使用繁琐，不支持直接RPC | 仅适合在不同App之间使用<br/>发生异常不回返回调用者<br/>必须使用Service，需要考虑兼容系统版本 | 不支持RPC<br>支持的数据格式过少,仅支持Bundle类的数据；Messenger能发送的信息只有Message能携带的信息。 | 不支持并发<br>无法实时通信                                   | 不能传递复杂消息，只能用来同步 |              |           |
| 本质                       | IO流                                                         | 传输层网络协议                                               |                                                              |                                                              |                                                              |                                |              |           |
